    ...
    <v-row>
      <v-col>
        ...
        <v-data-table :headers="headers" :items="ruleDetections" :items-per-page="10"
          ...
          <template v-slot:[`item.tx_ratio_yn`]="{ item }">
            <v-chip :color="getYnColor(item.tx_ratio_yn)" dark small>
              {{ item.tx_ratio_yn }}
            </v-chip>
          </template>
          <template v-slot:[`item.tx_err_ratio_yn`]="{ item }">
            <v-chip :color="getYnColor(item.tx_err_ratio_yn)" dark small>
              {{ item.tx_err_ratio_yn }}
            </v-chip>
          </template>
